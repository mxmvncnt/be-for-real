...
friend.post('/:friendId/accept', async (c) => {
	...
	await db
		.update(friendsTable)
		.set({ confirmed: 1 })
		.where(and(eq(friendsTable.userId1, friendId), eq(friendsTable.userId2, currentUserId)))
...
friend.post('/:friendId/remove', async (c) => {
	...
	await db
		.delete(friendsTable)
		.where(
			and(eq(friendsTable.userId1, existing.userId1), eq(friendsTable.userId2, existing.userId2)),
		)
...
friend.get('/requests/received', async (c) => {
	...
	const requestRows = await db
		.select()
		.from(friendsTable)
		.where(and(eq(friendsTable.userId2, currentUserId), eq(friendsTable.confirmed, 0)))
...
friend.get('/requests/sent', async (c) => {
	...
	const requestRows = await db
		.select()
		.from(friendsTable)
		.where(and(eq(friendsTable.userId1, currentUserId), eq(friendsTable.confirmed, 0)))
