...
export function RewindsPage() {
  ...
  const [compiledRewindUrls, setCompiledRewindUrls] = useState<
  ...
  const [activeMultiRewindId, setActiveMultiRewindId] = useState<string | null>(
    null,
  );
  const [rewindRenderError, setRewindRenderError] = useState<string | null>(
    null,
  );
  const [renderingRewindId, setRenderingRewindId] = useState<string | null>(
    null,
  );
  ...
  const dailyRewinds = useMemo<DailyRewind[]>(() => {
    ...
    return Array.from(groups.values())
      ...
      .sort(
        (left, right) =>
          new Date(
            right.clips[right.clips.length - 1]?.createdAt ?? 0,
          ).getTime() -
  ...
  const handleAddFriend = async (friend: Friend) => {
    ...
    try {
      ...
      setFriends((previous) => {
        if (
          previous.some((existingFriend) => existingFriend.id === friend.id)
        ) {
          return previous;
        }
      ...
      setFriendResults((previous) =>
        previous.filter((result) => result.id !== friend.id),
      );
    } catch (error) {
      console.error(error);
  ...
  const handleRemoveFriend = async (friend: Friend) => {
    ...
    try {
      ...
      setFriends((previous) =>
        previous.filter((existingFriend) => existingFriend.id !== friend.id),
      );
      setRewindFeed((previous) =>
        previous.filter((clip) => clip.userId !== friend.id),
      );
      setMultiRewinds((previous) =>
        previous.filter(
          (rewind) =>
            !rewind.participants.some(
              (participant) => participant.ownerId === friend.id,
            ),
        ),
      );
    } catch (error) {
    ...
    try {
      ...
      setMultiRewinds((previous) =>
        previous.map((existingRewind) =>
          existingRewind.id === rewind.id
            ? { ...existingRewind, videoUrl }
            : existingRewind,
        ),
      );
    } catch (error) {
    ...
    <>
      ...
      >
        <section className="rewinds-screen">
          ...
          <section className="rewinds-panel">
            <section className="friends-panel" aria-labelledby="friends-title">
              ...
              {friendError ? (
                <p className="friends-panel__message">{friendError}</p>
              ) : null}
            ...
            {activeTab === "rewinds" ? (
              <div className="rewinds-groups">
                {rewindsError ? (
                  <p className="friends-panel__message">{rewindsError}</p>
                ) : null}
                {rewindsLoading ? (
                  <p className="friends-panel__message">Loading rewinds...</p>
                ) : null}
                ...
                {filteredRewinds.map((rewind) => (
                  <section key={rewind.id} className="rewinds-group">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => void openRewind(rewind)}
                    >
                      <div className="rewinds-card__summary">
                        ...
                        <div className="rewinds-card__copy">
                          <strong>
                            {rewind.isYou
                              ? "Your rewind"
                              : `${rewind.ownerName}'s rewind`}
                          </strong>
                          <span>
                            {rewind.clipCount} clips compiled in order
                          </span>
                          <span>
                            Times:{" "}
                            {rewind.clips
                              .map((clip) => clip.timeLabel)
                              .join(" · ")}
                          </span>
                        </div>
                      </div>
              ...
              <div className="rewinds-groups">
                {filteredMultiRewinds.length === 0 ? (
                  <p className="friends-panel__message">
                    No Multi-Rewinds yet. Create one from the MIX button.
                  </p>
                ) : null}
                {filteredMultiRewinds.map((rewind) => (
                  <section key={rewind.id} className="rewinds-group">
                    ...
                    <button
                      className="rewinds-card rewinds-card--button"
                      type="button"
                      onClick={() => void openMultiRewind(rewind)}
                    >
                      <div className="rewinds-card__summary">
                        <div
        ...
        <aside className="rewinds-fab">
          ...
          <button
            className="rewinds-action rewinds-action--cyan"
            type="button"
      ...
      {activeRewind ? (
        ...
        >
          <div className="rewind-player">
            ...
            {rewindRenderError ? (
              <p className="friends-panel__message">{rewindRenderError}</p>
            ) : null}
      ...
      {activeMultiRewind ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="multi-rewind-title"
        >
          <div className="rewind-player">
            <div className="rewind-player__header">
              <div>
                ...
                <p>
                  {activeMultiRewind.participants.length} equal panels,
                  auto-advancing together
                </p>
              </div>
              <button
                className="composer-close"
                type="button"
                onClick={closeMultiRewind}
              >
                X
              </button>
            </div>
            ...
            {multiRenderError ? (
              <p className="friends-panel__message">{multiRenderError}</p>
            ) : null}
      ...
      {isComposerOpen ? (
        ...
        >
          <div className="composer-modal">
            ...
            {composerError ? (
              <p className="friends-panel__message">{composerError}</p>
            ) : null}
